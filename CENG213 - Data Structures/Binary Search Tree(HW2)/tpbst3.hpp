...
void TwoPhaseBST<T>::remove_helper2( const std::string &secondaryKey, SecondaryNode * &sNode, SecondaryNode * &sNode_parent , PrimaryNode * &pNode ) {
    ...
    if( sNode == NULL || sNode->key == "" ) {
        ...
        if( sNode->right == NULL && sNode->left == NULL ) {
			...
			else if( sNode_parent->right == sNode) {

				sNode_parent->right = sNode->left ;
			...
			else if( sNode_parent->left == sNode) {

				sNode_parent->left = sNode->left ;
        ...
        /*else if( sNode->right == NULL && sNode->left != NULL ) {
            ...
            sNode->right = sNode->left->right ;

            sNode->data  = sNode->left->data  ;

            sNode->key   = sNode->left->key   ;

            SecondaryNode *temp = sNode->left ;

            sNode->left  = temp->left         ;
        ...
        else remove_helper3( sNode , sNode->right , sNode_parent , pNode ) ; 
        
    }     
}

template <class T>
void TwoPhaseBST<T>::remove_helper3( SecondaryNode * &sNode , SecondaryNode * &sNode2 , SecondaryNode * &sNode_parent , PrimaryNode * &pNode ) {

    SecondaryNode * sNode2_parent = NULL ; 

    while( sNode2->left != NULL ) {

        sNode2_parent = sNode2 ;

        sNode2 = sNode2->left ;
    }

    if(sNode2_parent != NULL) 
        
        sNode2_parent->left = sNode2->right ;


    std::cout<<sNode->key<<std::endl ;

	if( sNode->right ) std::cout<<"snode right:"<<sNode->right->key<<std::endl ;

	if( sNode->left ) std::cout<<"snode left:"<<sNode->left->key<<std::endl ;	

	std::cout<<pNode->rootSecondaryNode->key<<std::endl ;

	if( pNode->rootSecondaryNode->right ) std::cout<<"rsn right:"<<pNode->rootSecondaryNode->right->key<<std::endl ;

	if( sNode2->right ) std::cout<<"snode2 right:"<<sNode2->right->key<<std::endl ;

	if( pNode->rootSecondaryNode->left ) std::cout<<"rsn left:"<<pNode->rootSecondaryNode->left->key<<std::endl ;

	if( sNode2->left ) std::cout<<"snode2 left:"<<sNode2->left->key<<std::endl ;


	std::cout<<"-------------------------------"<<std::endl;

	if( sNode->right != sNode2 ) {

		sNode2->right = sNode->right ; 

		}

	sNode2->left = sNode->left ;

    if( pNode->rootSecondaryNode == sNode ) {

    	pNode->rootSecondaryNode = sNode2 ;
    	
    }

    else if( sNode_parent->right == sNode ) { 

    	sNode_parent->right = sNode2 ;
    }

    else if( sNode_parent->left == sNode ) {

    	sNode_parent->left = sNode2 ;
    }
