...
# IADDQ
#
#IADDQ V,rB = C 0(first byte) F rB(second byte) V(other 8 bytes) (according to practice problem on page 405)
#
#FETCH STAGE  
#	icode:ifun <- M1[PC]    #icode is C ,ifun is 0 (first byte is taken)
#   rA:rB <- M1[PC+1]		#Note that rA is F that is R15(omitted , not exists in Y86-64 )
#   valC <- M8[PC+2]        #Read the immediate value (the 8 bytes described with V ) 
#	valP <- PC+10			#Since instruction size is 10 bytes,get PC+10 into the valP
#
#DECODE STAGE
#	valB <- R[rB]           #read the value in the register rB,put it into the valB
#
#EXECUTE STAGE
#	valE <- valB+valC		#valC in aluA,valB in aluB,sum of them in valE
#	Set CC                  #set condition flags ZF,SF,OF according to values                   
#
#MEMORY STAGE               #no operation exists in memory stage   
#
#WRITEBACK STAGE           
#	R[rB] <- valE           #Computed value valE put into the rB because rB is dest register. 
#
#PC UPDATE STAGE
#	PC <- valP              #Program counter updated and IADDQ is completed.
####################################################################
...
#IIADDQ is a valid instruction so it is added above. 
...
#IIADDQ instruction requires a rigid byte rB(rB is dest register).

# Does fetched instruction require a constant word?
bool need_valC =
	f_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IJXX, ICALL, IIADDQ };

#IIADDQ instruction requires a constant word.
...
##IIADDQ instruction requires register rB.
...
##IIADDQ instruction added because result E is written back to the rB.
...
##valC is loaded into the aluA,thus IIADDQ operation is added.
...
##valB is loaded into the aluB,thus IIADDQ operation is added.
...
##IIADDQ operation can set condition flags according to values.
